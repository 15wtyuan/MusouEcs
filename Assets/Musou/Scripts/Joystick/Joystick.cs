...
public class Joystick : MonoBehaviour, IPointerDownHandler, IDragHandler, IPointerUpHandler
{
    ...
    public bool showDirectionArrow; // 是否展示指向器
    [Space(8)] public JoystickEvent onValueChanged = new JoystickEvent(); //事件 ： 摇杆被 拖拽时
    ...
    void IPointerDownHandler.OnPointerDown(PointerEventData eventData)
    {
        ...
        if (dynamic)
        {
            ...
            pointerDownPosition[2] = eventData.pressEventCamera?.WorldToScreenPoint(position).z ??
                                     position.z;
            position = eventData.pressEventCamera?.ScreenToWorldPoint(pointerDownPosition) ??
                       pointerDownPosition;
